...
const DEFAULT_DRIP_EASING = function(t){
	if (t < 0)
		return 0;
	if (t > 1)
		return 1;
...
const getRandomRatio = function(value, ratio){
	ratio = Math.max(0, Math.min(1, ratio));
	return (value * (1 - ratio)) + (value * ratio * Math.random());
};

export default class GMLRenderContext {
	...
	setGml(gml) {
		...
		this.timeline = this.gml.getTimeline();
	}
	setOptions(options) {
		if (!options || typeof(options.hasOwnProperty) !== 'function')
			return;
		if (options.hasOwnProperty('dripFactor') && !isNaN(options.dripFactor))
			this.options.dripFactor = Math.min(1, Math.max(0, options.dripFactor));
		if (options.hasOwnProperty('dripLength') && !isNaN(options.dripLength))
			this.options.dripLength = Math.min(10, Math.max(0, options.dripLength));
		if (options.hasOwnProperty('dripSpeed') && !isNaN(options.dripSpeed))
			this.options.dripSpeed = Math.min(3600, Math.max(0, options.dripSpeed));
		if (options.hasOwnProperty('dripEasing') && typeof(options.dripEasing) === 'function')
	...
	renderDrips(context, gmlEnv, clientEnv, timeIndex, time) {
