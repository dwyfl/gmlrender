...
import { Timeline } from "./timeline/animation";
import { GML_WINDOW } from "./isomorphic/time";
import {
...
export type GMLViewEvents = (typeof GMLViewEvents)[keyof typeof GMLViewEvents];
...
export class GMLView extends EventEmitter {
  ...
  setGml(gml) {
    ...
    this.timeline.on(Timeline.EVENT_START, (event) =>
      this.emit(EVENT_START, event)
    );
    this.timeline.on(Timeline.EVENT_STOP, (event) =>
      this.emit(EVENT_STOP, event)
    );
  }
